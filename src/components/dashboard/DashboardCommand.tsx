...
import { AcuityPanel } from '@/components/dashboard/AcuityPanel';
    ...
    <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: '24px' }}>
        <Box>
          ...
          <Box sx={{ color: t.text.primary, fontSize: '1.25rem', fontWeight: 700, letterSpacing: '-0.02em', mt: '2px' }}>Clinical operations dashboard</Box>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        {[
          { label: 'Total Patients', value: patients.length, subtext: 'Active panel',         icon: Hospital,       accent: t.accents.cyan   },
          { label: 'High Risk',      value: highRisk,         subtext: 'Risk score ≥ 75',      icon: AlertTriangle,  accent: t.accents.red    },
          { label: 'Open Tasks',     value: openTasks,        subtext: 'Pending care tasks',   icon: Clock3,         accent: t.accents.orange },
        ].map((card) => (
          <Grid size={4} key={card.label}><InsightCard {...card} /></Grid>
        ))}
      </Grid>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          { label: 'Critical',    value: critical,                                                  subtext: 'Immediate attention', icon: ShieldCheck, accent: t.accents.purple },
          { label: 'Care Teams', value: 4,                                                          subtext: 'Active service lines', icon: Activity,    accent: t.accents.green  },
          { label: 'Recovering', value: patients.filter((p) => p.status === 'Recovering').length,  subtext: 'Post-acute patients',  accent: t.accents.blue   },
        ].map((card) => (
          <Grid size={4} key={card.label}><InsightCard {...card} /></Grid>
        ))}
      </Grid>
